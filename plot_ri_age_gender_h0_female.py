...
print '>> Paired T-Test RI^{[40-64],F} == null model'
tstat, pvalue = stats.ttest_rel( dfF.loc['40-44':'60-64','RI^{y}'] , dfN.loc['40-44':'60-64','RI^{y,F}_{rnd}'] )
...
ri_f_rnd, = ax.plot(ageinds, dfN['RI^{y,F}_{rnd}'].values, color='#ffcccc', marker='X', markersize=(ms/2)+2, linestyle='', zorder=6) #light red
ri_f_rnd_, = ax.fill(np.NaN, np.NaN, 'lightpink', edgecolor='lightgray', lw=1)
...
# x fold Annotation
ys, yf = dfF.loc['60-64','RI^{y}'].item(), dfN.loc['60-64','RI^{y,F}_{rnd}'].item()
diff = ys - yf
ymid = ( ( ys + yf )/2 )
ax.plot((12,12), (ys, yf), color='#00b200', lw=2)
...
ax.legend([ri_f,(ri_f_rnd_,ri_f_rnd)],[r'$RI^{[y_1,y_2],F}$',r'$RI^{[y_1,y_2],F\star} [H^{rnd}_0]$'],
	loc=2, handletextpad=0.5, columnspacing=0, handlelength=2, ncol=1)

ax.set_title(r'$RI^{[y_1,y_2],F}$')
...
plt.tight_layout()
plt.savefig('images/img-ri-age-gender-h0-female.pdf', dpi=300)
plt.close()
